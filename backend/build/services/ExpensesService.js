...
class ExpenseService {
  ...
  async getExpense() {
    return await Expense.findById(this._id)
      .populate('user')
      .populate('group')
      .exec();
  }
  ...
  async updateExpense(properties) {
    return await Expense.findByIdAndUpdate(this._id, properties, {
      new: true,
    }).exec();
  }
  ...
  static convertToCSV(expenses) {
    ...
    for (let expense of expenses) {
      ...
      row +=
        expense.createdAt.toLocaleDateString('sl-SL') +
        expense.createdAt.toLocaleTimeString('sl-SL');
      +',';
      row +=
        expense.updatedAt.toLocaleDateString('sl-SL') +
